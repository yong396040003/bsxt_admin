...
layui.use(['table', 'jquery', 'layer', 'laydate', 'upload', 'form'], function() {
    ...
    var tableIns = table.render({
        ...
        cols: [
            [
                ...
                { field: 'ProductId', title: 'ProductId', align: 'center', fixed: true },
                { field: 'From', title: 'From', align: 'center', fixed: true },
                { field: 'To', title: 'To', align: 'center', fixed: true },
                { field: 'CreateTime', title: 'CreateTime', align: 'center', fixed: true },
                { field: 'UpdateTime', title: 'UpdateTime', align: 'center', fixed: true },
    ...
    table.on('tool(Book)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        switch (layEvent) {
            case 'del':
                layer.msg('<br>是否确认删除<br>' + data.ProductId + '', {
                    time: 20000, //20s后自动关闭
                    btn: ['狠心删除', '考虑考虑'],
                    btn1: function(index) {
                        $.ajax({
                            type: 'get',
                            url: 'http://localhost:8888/deleteTransaction?code=' + data.ProductId,
                            dataType: 'json',
                            success: function(res) {
                                if (res.status == 1) {
                                    obj.del();
    ...
    function addNews(edit, data) {
        if (edit == 'edit') {
            var index = layer.open({
                type: 2,
                title: '修改记录',
                content: 'editJiLu.html?code=' + data.ProductId,
                shade: 0,
    ...
    function search() {
        ...
        if (input == '') {
            ...
            tableIns.reload({
                ...
                url: 'http://localhost:8888/getRoute?code=' + input,
                method: 'get',
                text: {
                    none: '没有找到你想要的'
                },
